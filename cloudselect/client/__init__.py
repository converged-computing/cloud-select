...
import cloudselect
import cloudselect.cloud as cloud
...
cloud-select config add cloud aws""",
    ...
    # Add cloud selection to all commands
    for command in [instance, shell, cache, config, dbshell]:
        command.add_argument(
            "--cloud",
            dest="clouds",
        ...
        # Default is None so we can refer to settings
        command.add_argument(
            "--no-cache",
            help="one-off disable of using the cache",
            action="store_true",
            default=None,
        )
